...
router.get("/owner/:ownerId", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Terjadi kesalahan saat mengambil produk berdasarkan pemilik.",
        error,
      });
  }
});
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        ...
        .json({
          message:
            "Terjadi kesalahan saat mengambil produk berdasarkan pemilik.",
          error,
        });
    }
  }
...
router.post("/", async (req: Request, res: Response) => {
  try {
    const { name, description, price, productCategory, thumbnails, owner } =
      req.body;
    ...
    res
      .status(500)
      .json({ message: "Terjadi kesalahan saat membuat produk baru.", error });
  }
});
...
router.get("/:id", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Terjadi kesalahan saat mengambil produk.", error });
  }
});
...
router.delete("/:id", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Terjadi kesalahan saat menghapus produk.", error });
  }
});
